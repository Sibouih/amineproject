...
class PurchasesController extends BaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $purchases['total_amount'] = DB::table('purchases')
        ->where('deleted_at', '=', null)
        ->where('statut', 'received')
        ->where('provider_id', $purchase['provider']->id)
        ->sum('GrandTotal');

        $purchases['total_paid'] = DB::table('purchases')
            ->where('deleted_at', '=', null)
            ->where('statut', 'received')
            ->where('provider_id', $purchase['provider']->id)
            ->sum('paid_amount');

        $purchases['due'] = $purchases['total_amount'] - $purchases['total_paid'];
        $total_credit = $purchase['provider']->credit_initial + $purchases['due'];
        ...
        $purchase_data['total_credit'] = number_format($total_credit, 2, '.', '');
    ...
    public function Purchase_pdf(Request $request, $id)
    {
        ...
        $purchases['total_amount'] = DB::table('purchases')
            ...
            ->where('provider_id', $Purchase_data['provider']->id)
            ->sum('GrandTotal');
        ...
        $purchases['due'] = $purchases['total_amount'] - $purchases['total_paid'];
        $total_credit = $Purchase_data['provider']->credit_initial + $purchases['due'];
        ...
        $purchase['total_credit'] = number_format($total_credit, 2, '.', '');
