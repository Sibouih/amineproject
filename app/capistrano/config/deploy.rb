...
set :application, "inwi-diriddik"
set :repo_url, "git@bitbucket.org:adminvoid/inwi-diriddik.git"
set :default_env, {
  'http_proxy' => 'http://10.104.88.7:80',
  'https_proxy' => 'http://10.104.88.7:80',
  'HTTP_PROXY' => 'http://10.104.88.7:80',
  'HTTPS_PROXY' => 'http://10.104.88.7:80'
}
...
SSHKit.config.command_map[:drush] = "/home/void/.config/composer/vendor/bin/drush"
...
namespace :vactory8 do
   ...
   task :apply_permissions do
       on roles(:app) do
           ...
           within release_path.join(fetch(:app_path)) do
               ...
               execute :find, "#{release_path}/vendor/drush/drush/drush", " -exec chmod u=rx,g=r,o= '{}' ';'"
               ...
               execute "chmod u=rwx,g=rx,o=rx #{release_path}/private"
           end
       end
   end
  ...
  before 'deploy:starting', 'composer2:install'
