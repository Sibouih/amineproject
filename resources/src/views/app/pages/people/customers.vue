          ...
          <span v-if="props.column.field == 'actions'">
            <div>
              ...
              >
                ...
                <b-dropdown-item
                  v-if="props.row.total_credit > 0 && currentUserPermissions && currentUserPermissions.includes('pay_due')"
        ...
        <b-form @submit.prevent="Submit_Payment_sell_due">
          <b-row>
            ...
            <b-col lg="12" md="12" sm="12">
              <validation-provider name="Due Type" :rules="{ required: true}">
                <b-form-group slot-scope="{ valid, errors }" :label="$t('Due_Type')+ ' ' + '*'">
                  <v-select
                    ...
                    :options="
                          ...
                          {label: $t('Seulement_credit_ventes'), value: 'credit_ventes_only'},
                          {label: $t('Credit_ventes_apres_credit_initial'), value: 'credit_ventes_first'},
                          {label: $t('Credit_initial_apres_credit_ventes'), value: 'credit_initial_first'},
                          ]"
                  ></v-select>
            ...
            <b-col lg="6" md="12" sm="12">
              ...
              >
                <b-form-group :label="$t('Paying_Amount') + ' ' + '*'">
                  ...
                  <span class="badge badge-danger">{{$t('Due')}} : {{currentUser.currency}} {{payment.total_credit}}</span>
                  <ul>
                    <li><span class="badge badge-success">{{$t('Credit_Ventes')}} : {{currentUser.currency}} {{payment.due}}</span></li>
                    <li><span class="badge badge-success">{{$t('Credit_Initial')}} : {{currentUser.currency}} {{payment.credit_initial}}</span></li>
                  </ul> 
                </b-form-group>
              </validation-provider>
            </b-col>
        ...
        <b-form @submit.prevent="Submit_Customer">
          <b-row>
            ...
            <b-col md="6" sm="12">
             ...
             <!-- Credit initial -->
            <b-col md="6" sm="12">
                <b-form-group :label="$t('Credit_Initial')">
                  <b-form-input
                    label="Credit Initial"
                    v-model="client.credit_initial"
        ...
        <b-col lg="12" md="12" sm="12" class="mt-3">
          <table class="table table-striped table-md">
            <tbody>
              ...
              <tr>
                <!-- Customer Credit Initial -->
                <td>{{$t('Credit_Initial')}}</td>
                <th>{{currentUser.currency}} {{client.credit_initial}}</th>
              </tr>
              <tr>
                <!-- Customer Adress -->
                <td>{{$t('Adress')}}</td>
  ...
  methods: {
    ...
    Create_Client() {
      ...
      axios
        .post("clients", {
          ...
          remise: this.client.remise ?? 0,
          credit_initial: this.client.credit_initial,
    ...
    Submit_Payment_sell_due() {
      this.$refs.ref_pay_due.validate().then(success => {
        ...
        } else if (this.payment.due_type == '' && this.payment.amount > this.payment.due) {
          this.makeToast(
            "warning",
    ...
    Verified_paidAmount() {
      if (isNaN(this.payment.amount)) {
        ...
        let warning = false
        let warning_message = "Paying_amount_is_greater_than_Total_Due"
        if (this.payment.due_type == 'credit_initial_only' && this.payment.amount > this.payment.credit_initial) {
          warning = true
          warning_message = "Paying_amount_is_greater_than_Credit_Initial"

        }
        else if (this.payment.due_type == 'credit_ventes_only' && this.payment.amount > this.payment.due) {
          warning = true
        }
        else if ((this.payment.due_type == 'credit_ventes_first' || this.payment.due_type == 'credit_initial_first') && this.payment.amount > (this.payment.due + this.payment.credit_initial)) {
          warning = true
          warning_message = "Paying_amount_is_greater_than_Total_Credit"
        }

        if (warning) {
          this.makeToast(
            "warning",
            this.$t(warning_message),
            this.$t("Warning")
          );
          this.payment.amount = 0;
        } 
